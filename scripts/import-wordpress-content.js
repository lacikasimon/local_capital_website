...
async function main() {
  ...
  for (const language of LANGUAGES) {
    ...
    const wpPosts = fetchJson(`${BASE_URL}/wp-json/wp/v2/posts?per_page=100&lang=${language}`);
    for (const post of wpPosts) {
      ...
      posts.push({
        ...
        source_type: "post",
        slug: post.slug,
        path: pathnameFromUrl(post.link) || `/blog/${post.slug}`,
        source_url: post.link,
        title: cleanText(post.title?.rendered ?? post.slug),
        post_date: post.date?.slice(0, 10) || "2026-04-29",
        excerpt: excerpt(cleanText(post.excerpt?.rendered ?? body)),
        body,
        published: 1
      });
      htmlUrls.add(post.link);
    }
  }
